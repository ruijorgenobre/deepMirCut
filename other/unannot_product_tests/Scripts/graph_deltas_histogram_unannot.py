...
if __name__ == "__main__":
    if (len(sys.argv) < 6):
        print("USAGE: %s <5p unanot dataset file> <3p unanot dataset file> <5p unanot decision values file> <3p unannot decision values file> <output prefix>\n"%(sys.argv[0]))
        exit()
    unannot_5p_dataset_file = sys.argv[1]
    unannot_3p_dataset_file = sys.argv[2]
    unannot_5p_pred_file = sys.argv[3]
    unannot_3p_pred_file = sys.argv[4]
    output_prefix = sys.argv[5]

    delta_range=[-9,9]

    cuts_5p = read_cuts_file(unannot_5p_dataset_file)
    data_5p = read_predicted_values_file(unannot_5p_pred_file)
    max_dvs_5p, max_positions_5p = get_max_dvs(data_5p)
    cutsite_dvs_5p = get_cutsite_dvs(cuts_5p,data_5p)
    deltas_5p = get_position_difference(cuts_5p,max_positions_5p)
    delta_counts_5p = prepare_delta_boxplot_data(deltas_5p,delta_range=delta_range)
    delta_counts_5p = normalize_deltaCounts(delta_counts_5p,size=len(cuts_5p))

    cuts_3p = read_cuts_file(unannot_3p_dataset_file)
    data_3p = read_predicted_values_file(unannot_3p_pred_file)
    max_dvs_3p, max_positions_3p = get_max_dvs(data_3p)
    cutsite_dvs_3p = get_cutsite_dvs(cuts_3p,data_3p)
    deltas_3p = get_position_difference(cuts_3p,max_positions_3p)
    delta_counts_3p = prepare_delta_boxplot_data(deltas_3p,delta_range=delta_range)
    delta_counts_3p = normalize_deltaCounts(delta_counts_3p,size=len(cuts_3p))


    delta_counts = {'DR5' : delta_counts_5p['DR5'],
                    'DC5' : delta_counts_5p['DC5'],
                    'DC3': delta_counts_3p['DC3'],
                    'DR3': delta_counts_3p['DR3']}
    ...
    cutsite_linewidth = 2
    dot_linewidth = 1
    plt.xlabel("Positional Shift of Cut Site Predictions\nRelative to Annotations",fontsize=14)
    plt.ylabel("Frequency of Predictions\n(Best Replicate Applied to Test Set)",fontsize=14)
    for x in vertical_line_lengths:
        plt.plot([int(x),int(x)],[0,vertical_line_lengths[x]],linewidth=0.5,linestyle='-',color="black");
    for c in cutsites:
        plt.plot(lb_xVals,lb_yVals[c],linewidth=cutsite_linewidth,linestyle='-',color="white")
        plt.plot([lb_xVals[-1],lb_xVals[-1]],[0,lb_yVals[c][1]],linewidth=cutsite_linewidth,linestyle='-',color="white")
        plt.plot([lb_xVals[-1],hist_xVals[0]],[lb_yVals[c][-1],hist_yVals[c][0]],linewidth=dot_linewidth,linestyle=':',color="white")
        plt.plot([hist_xVals[0],hist_xVals[0]],[0,hist_yVals[c][0]],linewidth=cutsite_linewidth,linestyle='-',color="white")
        plt.plot(hist_xVals,hist_yVals[c],linewidth=cutsite_linewidth,linestyle='-',color="white")
        plt.plot([hist_xVals[-1],hist_xVals[-1]],[0,hist_yVals[c][-1]],linewidth=cutsite_linewidth,linestyle='-',color="white")
        plt.plot([hist_xVals[-1],ub_xVals[0]],[hist_yVals[c][-1],ub_yVals[c][-1]],linewidth=dot_linewidth,linestyle=':',color="white")
        plt.plot([ub_xVals[0],ub_xVals[0]],[0,ub_yVals[c][0]],linewidth=cutsite_linewidth,linestyle='-',color="white")
        plt.plot(ub_xVals,ub_yVals[c],linewidth=cutsite_linewidth,linestyle='-',color="white")
    for c in cutsites:
        plt.plot(lb_xVals,lb_yVals[c],linewidth=cutsite_linewidth,linestyle='-',color=colors[c],alpha=0.5)
        plt.plot([lb_xVals[-1],lb_xVals[-1]],[0,lb_yVals[c][1]],linewidth=cutsite_linewidth,linestyle='-',color=colors[c],alpha=0.5)
        plt.plot([lb_xVals[-1],hist_xVals[0]],[lb_yVals[c][-1],hist_yVals[c][0]],linewidth=dot_linewidth,linestyle=':',color=colors[c],alpha=0.5)
        plt.plot([hist_xVals[0],hist_xVals[0]],[0,hist_yVals[c][0]],linewidth=cutsite_linewidth,linestyle='-',color=colors[c],alpha=0.5)
        plt.plot(hist_xVals,hist_yVals[c],linewidth=cutsite_linewidth,linestyle='-',color=colors[c],alpha=0.5)
        plt.plot([hist_xVals[-1],hist_xVals[-1]],[0,hist_yVals[c][-1]],linewidth=cutsite_linewidth,linestyle='-',color=colors[c],alpha=0.5)
        plt.plot([hist_xVals[-1],ub_xVals[0]],[hist_yVals[c][-1],ub_yVals[c][-1]],linewidth=dot_linewidth,linestyle=':',color=colors[c],alpha=0.5)
        plt.plot([ub_xVals[0],ub_xVals[0]],[0,ub_yVals[c][0]],linewidth=cutsite_linewidth,linestyle='-',color=colors[c],alpha=0.5)
        plt.plot(ub_xVals,ub_yVals[c],linewidth=cutsite_linewidth,linestyle='-',color=colors[c],alpha=0.5)
    legend_lines = [Line2D([0], [0], color=colors[c], linewidth=cutsite_linewidth, linestyle='-', alpha=0.5) for c in cutsites]
    ...
    plt.xticks(tickLocations, ["less\nthan %s" % (str(delta_range[0]))] + keys[1:-1] + ["more\nthan %s" % (str(delta_range[1]))])
    plt.ylim(ymin=0)
    plt.xlim(xmin=0,xmax=ub_xVals[1])
