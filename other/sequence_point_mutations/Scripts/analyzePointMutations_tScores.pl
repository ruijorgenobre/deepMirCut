...
my $usage = "USAGE:\n$0 <point mutation file> [<train Freq File> <min training frequency>]\n";

my $pointMutationFile = $ARGV[0] or die $usage;
my $trainFreqFile = $ARGV[1] ? $ARGV[1] : "";
my $trainFreqMin = $ARGV[2] ? int($ARGV[2]) : 0;

my $tScoreOutputFile = ($trainFreqMin) ? $pointMutationFile . "\_stats\_wTScore\_minTrainFreq$trainFreqMin.txt" : $pointMutationFile . "\_stats\_wTScore.txt";
my $pValOutputFile = ($trainFreqMin) ? $pointMutationFile . "\_stats\_wTScore\_minTrainFreq$trainFreqMin\_pvals.txt" : $pointMutationFile . "\_stats\_wTScore\_pvals.txt";

my @bases = ("A","C","G","U");

my $pointMutationDeltas = readPointMutationFile($pointMutationFile);
my $trainFreq = ($trainFreqFile) ? readTrainFreqFile($trainFreqFile,\@bases) : 0;
my($calculations,$maxTScore) = runCalculations($pointMutationDeltas,\@bases,$trainFreq,$trainFreqMin);
...
sub runCalculations {
    my($pointMutationDeltas,$bases,$trainFreq,$trainFreqMin) = @_;
    my @calculations;
    my $maxTScore = -~0;
    my $maxMean = -~0;
    ...
    my $count = 0;
    if ($trainFreqMin > 0) {
	foreach my $pos (sort {$a <=> $b} keys %{$trainFreq}) {
	    foreach my $base (@{$bases}) {
		if ($trainFreq->{$pos}{$base} > $trainFreqMin) {
		    $count += 1;
		}
	    }
	}
	...
	for (my $bIdx = 0; $bIdx < @{$bases}; $bIdx++) {
	    ...
	    my($tValue,$deg_freedom,$pValue,$qValue) = ('-','-','-','-');
	    if ($trainFreqMin == 0 || $trainFreq->{$pos}{$base} > $trainFreqMin) {
		...
		($tValue,$deg_freedom) = $t_test->perform_t_test('after','before');
		my $cdf = Statistics::Distributions::tprob($deg_freedom,$tValue);
		$pValue = 2 * min($cdf,1-$cdf);
		$qValue = $pValue * $count; #Bonferroni corrected p-value
...
sub readTrainFreqFile {
    my($trainFreqFile,$bases) = @_;
    my %trainFreq;
    open(TFFLE,$trainFreqFile) or die "failed to open $trainFreqFile";
    while (<TFFLE>) {
	chomp;
	unless ( /^#/ ) {
	    my($pos,$baseId,$freq) = split(/\t/);
	    my $base = $bases->[$baseId];
	    if ($trainFreq{$pos}{$base}) {
		die "Error: more than one entry of $pos $baseId in $trainFreqFile";
	    }
	    $trainFreq{$pos}{$base} = $freq;
	}
    }
    close(TFFLE);
    return \%trainFreq;
