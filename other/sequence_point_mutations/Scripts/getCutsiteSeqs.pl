...
sub getSeqsFromDataset {
	...
	chomp;
	unless ( /^#/ ) {
	    my($id,$name,$miRBaseId,$prod5p,$prod3p,$DR5,$DC5,$DC3,$DR3,$hpStart,$hpStop,$seq) = split(/\t/);
	    #Drosha 5p cutsite
	    my($DR5LeftN,$DR5SeqStart,$DR5SeqStop,$DR5RightN) = getSeqSpan($DR5,$seq,$parameters);
	    my $DR5Seq = 'N' x $DR5LeftN . substr($seq,$DR5SeqStart,$DR5SeqStop-$DR5SeqStart+1) . 'N' x $DR5RightN;
	    ...
	    my $DC5Seq = 'N' x $DC5LeftN . substr($seq,$DC5SeqStart,$DC5SeqStop-$DC5SeqStart+1) . 'N' x $DC5RightN;
	    ...
	    my $DC3Seq = 'N' x $DC3LeftN . substr($seq,$DC3SeqStart,$DC3SeqStop-$DC3SeqStart+1) . 'N' x $DC3RightN;
	    ...
	    my $DR3Seq = 'N' x $DR3LeftN . substr($seq,$DR3SeqStart,$DR3SeqStop-$DR3SeqStart+1) . 'N' x $DR3RightN;
