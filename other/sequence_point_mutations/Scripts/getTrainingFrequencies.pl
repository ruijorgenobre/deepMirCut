...
my($countsDR5,$percentDR5) = getNucCounts($unmutatedDR5nucs,$parameters);
my($countsDC5,$percentDC5) = getNucCounts($unmutatedDC5nucs,$parameters);
my($countsDC3,$percentDC3) = getNucCounts($unmutatedDC3nucs,$parameters);
my($countsDR3,$percentDR3) = getNucCounts($unmutatedDR3nucs,$parameters);
...
sub printOutputFile {
    ...
    foreach my $baseChar (@bases) {
 	$linesDR5 .= $baseChar;
 	for (my $pos = -$parameters->{"downstreamDist"} + 1; $pos <= $parameters->{"upstreamDist"}; $pos++) { 
	    if ($countsDR5->{$baseChar}{$pos} =~ /\D/) {
		$linesDR5 .= sprintf("\t%.3f",$countsDR5->{$baseChar}{$pos});
	    } else {
		$linesDR5 .=  "\t$countsDR5->{$baseChar}{$pos}";
	    }
 	}
	$linesDR5 .= "\n";
    }
    ...
    foreach my $baseChar (@bases) {
 	$linesDC5 .= $baseChar;
 	for (my $pos = -$parameters->{"downstreamDist"} + 1; $pos <= $parameters->{"upstreamDist"}; $pos++) { 
	    if ($countsDC5->{$baseChar}{$pos} =~ /\D/) {
		$linesDC5 .= sprintf("\t%.3f",$countsDC5->{$baseChar}{$pos});
	    } else {
		$linesDC5 .= "\t$countsDC5->{$baseChar}{$pos}";
	    }
 	}
	$linesDC5 .= "\n";
    }
    ...
    foreach my $baseChar (@bases) {
 	$linesDC3 .= $baseChar;
 	for (my $pos = -$parameters->{"downstreamDist"} + 1; $pos <= $parameters->{"upstreamDist"}; $pos++) { 
	    if ($countsDC3->{$baseChar}{$pos} =~ /\D/) {
		$linesDC3 .= sprintf("\t%.3f",$countsDC3->{$baseChar}{$pos});
	    } else {
		$linesDC3 .= "\t$countsDC3->{$baseChar}{$pos}";
	    }
 	}
	$linesDC3 .= "\n";
    }
    ...
    foreach my $baseChar (@bases) {
 	$linesDR3 .= $baseChar;
 	for (my $pos = -$parameters->{"downstreamDist"} + 1; $pos <= $parameters->{"upstreamDist"}; $pos++) {
	    if ($countsDR3->{$baseChar}{$pos} =~ /\D/) {
		$linesDR3 .= sprintf("\t%.3f",$countsDR3->{$baseChar}{$pos});
	    } else {
		$linesDR3 .= "\t$countsDR3->{$baseChar}{$pos}";
	    }
 	}
	$linesDR3 .= "\n";
    }
...
sub getNucCounts {
    my($unmutatedNuc,$parameters) = @_;
    my $count = 0;
    my %unmutatedCount;
    my %unmutatedPercent;
    foreach my $baseId (keys %{$unmutatedNuc}) {
	$count += 1;
	for my $pos (sort {$a <=> $b} keys %{$unmutatedNuc->{$baseId}}) {
	    my $baseChar = $unmutatedNuc->{$baseId}{$pos};
	    if ($unmutatedCount{$baseChar}{$pos}) {
		$unmutatedCount{$baseChar}{$pos} += 1;
	    } else {
		$unmutatedCount{$baseChar}{$pos} = 1;
	    }
	}
    }
    foreach my $baseChar (keys %unmutatedCount) {
	foreach my $pos (keys %{$unmutatedCount{$baseChar}}) {
	    $unmutatedPercent{$baseChar}{$pos} = $unmutatedCount{$baseChar}{$pos} / $count;
	}
    }    
