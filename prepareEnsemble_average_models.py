...
def print_usage():
    ...
    print("-n, --ensemble_size         prepare ensemble of this size")
...
if __name__ == "__main__":
    ...
    try:
        ...
        opts_array, args = getopt.getopt(sys.argv[min_args:], "hvdo:e:m:i:L:n:", ["help","verbose","print_dvs","output_prefix=","epochs=","model=","input_setting=","max_seq_len=",
                                                                               "use_embedding_layer=","use_crf_layer=","embedding_layer_output=","embedding_dropout=","bi_lstm1_units=","bi_lstm2_units=",
                                                                                  "dense1_units=","dense2_units=","ensemble_list=","--ensemble_size="])
        if len(args) > 0:
    ...
    macro_f1 = sum([perf[c]["fscore"] for c in ["DR5","DC5","DC3","DR3"]]) / 4
    for c in ["DR5","DC5","DC3","DR3"]:
        print(c + "=", perf[c]["fscore"])
    print("average=",macro_f1)
