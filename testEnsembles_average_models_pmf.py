...
if __name__ == "__main__":
    ...
    output_file = "ensemble_unweighted_pmf.txt"
    if parameters["output_prefix"]:
        output_file = parameters["output_prefix"] + "_unweighted_pmf.txt"

    single_scores_output_file = "ensemble_unweighted_pmf_single_models.txt"
    if parameters["output_prefix"]:
        single_scores_output_file = parameters["output_prefix"] + "_unweighted_pmf_single_models.txt"
    ...
    for i in range(0,len(members)):
        ...
        cs_pmf_scores.append({c:perf[c]["pmf"] for c in ["DR5","DC5","DC3","DR3"]})
        avg_pmf_scores.append(sum([perf[c]["pmf"] for c in ["DR5","DC5","DC3","DR3"]]) / 4)
    ...
    for c in ["DR5","DC5","DC3","DR3"]:
        f_ss.write("\t%s"%(c))
    f_ss.write("\tavg_pmf\n")
    for _,i in sorted(zip(avg_pmf_scores,range(0,len(avg_pmf_scores))), reverse=True):
        f_ss.write(parameters["ensemble"][i])
        for c in ["DR5","DC5","DC3","DR3"]:
            f_ss.write("\t%f"%(cs_pmf_scores[i][c]))
        f_ss.write("\t%f\n"%(avg_pmf_scores[i]))
    f_ss.close()
    ...
    for i in range(0,len(test_pred)):
        ...
        for _,j in sorted(zip(avg_pmf_scores,range(0,len(avg_pmf_scores))), reverse=True)[:num]:
        ...
        avg_pmf = sum([perf[c]["pmf"] for c in ["DR5","DC5","DC3","DR3"]]) / 4
        print("ensemble",[perf[c]["pmf"] for c in ["DR5","DC5","DC3","DR3"]],avg_pmf)
        print()
        
        f.write("%d"%(num))
        for c in ["DR5","DC5","DC3","DR3"]:
            f.write("\t%f"%(perf[c]["pmf"]))
        f.write("\t%f\n"%(avg_pmf))
    f.close()
